    ...
    for (let model in sortedModelsTree) {
      for (let attribute in modelsTree[model]) {
        ...
        if (modelsTree[model][attribute].collection) {
          ...
          if (!intermediateTables[`${attributeCollection}_${attributeVia}__${model}_${attribute}`] &&
            !intermediateTables[`${attributeCollection}_${attribute}__${model}_${attributeVia}`]) {
            ...
            if (associationType === "many-to-many") {
              ...
              // process many-to-many self-connection (check which field comes first alphabetically and swap if attributeVia comes first)
              if (attributeCollection === model && attributeVia < attribute) {
                // create reverse table for many-to-many association
                intermediateTables[`${model}_${attributeVia}__${attributeCollection}_${attribute}`] = {
                  id: {type: 'serial', autoIncrement: true, primaryKey: true},
                  [`${model}_${attributeVia}`]: {type: modelPrimaryKeyType},
                  [`${attributeCollection}_${attribute}`]: {type: collectionPrimaryKeyType}
                }
